...
int main(int argc, char **argv) {
  ...
  //calculation parallel critical
  #pragma omp parallel private(cx, cy, x, y, x2, y2)  
  {
    int nthreads = omp_get_num_threads();
    int t_id = omp_get_thread_num();
    int j_beg = t_id * IMAGE_HEIGHT / nthreads;
    int j_end = (t_id + 1) * IMAGE_HEIGHT / nthreads;
    long localTotalIterationsCount = 0;
    ...
    #pragma omp critical
    nTotalIterationsCount += localTotalIterationsCount; 
